define("ace/keyboard/emacs", ["require", "exports", "module", "ace/lib/dom", "ace/incremental_search", "ace/commands/incremental_search_commands", "ace/keyboard/hash_handler", "ace/lib/keys"], function (e, t, n) {
	...
	t.handler.attach = function (e) {
		u || (u = !0, r.importCssString("            .emacs-mode .ace_cursor{                border: 2px rgba(50,250,50,0.8) solid!important;                -moz-box-sizing: border-box!important;                -webkit-box-sizing: border-box!important;                box-sizing: border-box!important;                background-color: rgba(0,250,0,0.9);                opacity: 0.5;            }            .emacs-mode .ace_cursor.ace_hidden{                opacity: 1;                background-color: transparent;            }            .emacs-mode .ace_overwrite-cursors .ace_cursor {                opacity: 1;                background-color: transparent;                border-width: 0 0 2px 2px !important;            }            .emacs-mode .ace_text-layer {                z-index: 4            }            .emacs-mode .ace_cursor-layer {                z-index: 2            }", "emacsMode")), a = e.session.$selectLongWords, e.session.$selectLongWords = !0, f = e.session.$useEmacsStyleLineStart, e.session.$useEmacsStyleLineStart = !0, e.session.$emacsMark = null, e.emacsMarkMode = function () {
			return this.session.$emacsMark
		}, e.setEmacsMarkMode = function (e) {
			this.session.$emacsMark = e
		}, e.on("click", c), e.on("changeSession", l), e.renderer.screenToTextCoordinates = s, e.setStyle("emacs-mode"), e.commands.addCommands(v), t.handler.platform = e.commands.platform, e.$emacsModeHandler = this
	}, t.handler.detach = function (e) {
		delete e.renderer.screenToTextCoordinates, e.session.$selectLongWords = a, e.session.$useEmacsStyleLineStart = f, e.removeEventListener("click", c), e.removeEventListener("changeSession", l), e.unsetStyle("emacs-mode"), e.commands.removeCommands(v)
	};
	var l = function (e) {
		e.oldSession && (e.oldSession.$selectLongWords = a, e.oldSession.$useEmacsStyleLineStart = f), a = e.session.$selectLongWords, e.session.$selectLongWords = !0, f = e.session.$useEmacsStyleLineStart, e.session.$useEmacsStyleLineStart = !0, e.session.hasOwnProperty("$emacsMark") || (e.session.$emacsMark = null)
	}, c = function (e) {
	...
	d.forEach(function (e) {
		...
		var i = e.editor;
		if (t == -1) {
			i.setEmacsMarkMode(null);
			if (e.count) {
				var s = Array(e.count + 1).join(n);
		...
		var a = this.commmandKeyBinding[n];
		e.keyChain = a == "null" ? n : "";
		if (!a)return undefined;
		if (a === "null")return{command: "null"};
		if (a === "universalArgument")return e.universalArgument = !0, {command: "null"};
		var f;
		typeof a != "string" && (f = a.args, a.command && (a = a.command), a === "goorselect" && (a = i.emacsMarkMode() ? f[1] : f[0], f = null));
		if (typeof a == "string") {
			(a === "insertstring" || a === "splitline" || a === "togglecomment") && i.setEmacsMarkMode(null), a = this.commands[a] || i.commands.commands[a];
			if (!a)return undefined
		}
		...
		var t = e.emacsMarkMode();
		if (t) {
			var n = e.getCursorPosition();
			if (e.selection.isEmpty() && t.row == n.row && t.column == n.column) {
				e.setEmacsMarkMode(null);
				return
			}
		}
		t = e.getCursorPosition(), e.setEmacsMarkMode(t), e.selection.setSelectionAnchor(t.row, t.column)
		...
		e.selection.clearSelection(), e.setEmacsMarkMode(null)
	...
	r.inherits(f, s), function () {
		this.activate = function (e, t) {
			...
			var t = this.$editor;
			return this.$prevNeedle = this.$options.needle, this.$options.needle = "", e && (t.moveCursorToPosition(this.$startPos), this.$currentPos = this.$startPos), this.highlight(null), i.fromPoints(this.$currentPos, this.$currentPos)
