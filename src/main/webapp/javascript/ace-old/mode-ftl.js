define("ace/mode/ftl", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text", "ace/tokenizer", "ace/mode/ftl_highlight_rules"], function (e, t, n) {
	...
	var u = function () {
		...
		i.call(this);
		for (var t in this.$rules)this.$rules[t].unshift({token: "string.interpolated", regex: "\\${",push:"ftl-start"},{token:"keyword.function",regex:"</?#("+e+")",push:"ftl-start"},{token:"keyword.other",regex:"</?@[a-zA-Z\\.]+",push:"ftl-start"});
		this.embedRules(o, "ftl-"), this.$rules["ftl-start"].unshift({token: "keyword", regex: "/?>", next: "pop"}, {token: "string.interpolated", regex: "}", next: "pop"}), this.$rules.start.unshift({token: "comment", regex: "<#--", next: "comment"}), this.$rules.comment.unshift({token: "comment", regex: ".*?-->", next: "start"}, {token: "comment", regex: ".+"}), this.normalizeRules()
	};
